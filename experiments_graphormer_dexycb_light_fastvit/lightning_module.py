...
from experiments_graphormer_dexycb_light_fastvit.pl_system_v6_graphormer import (
    PoseLightningModule,
)
...
class DexYCBPoseLightningModule(PoseLightningModule):
    """
    DexYCB wrapper around the Light FastViT Graphormer LightningModule.

    The upstream training_step/validation_step/test_step assume extra fields
    The batch contract contains RGB, 2D/3D joints, reconstructed MANO vertices,
    camera intrinsics and explicit joint validity.
    """
    root_joint_idx = 0
    hand_bones = (
        (0, 1), (1, 2), (2, 3), (3, 4),
    ...
    def _compute_masked_pa_metric(self, pred, gt, valid_mask=None):
        if valid_mask is None:
            return super()._compute_pa_mpjpe_3d(pred, gt)
        values = []
        valid_mask = valid_mask.to(device=pred.device, dtype=torch.bool)
        for sample_pred, sample_gt, sample_valid in zip(
            pred, gt, valid_mask
        ):
            finite = (
                sample_valid
                & torch.isfinite(sample_pred).all(dim=-1)
                & torch.isfinite(sample_gt).all(dim=-1)
            )
            if finite.sum() >= 3:
                values.append(
                    super()._compute_pa_mpjpe_3d(
                        sample_pred[finite][None], sample_gt[finite][None]
                    )
                )
        if values:
            return torch.stack(values).mean()
        return pred.nan_to_num().sum() * 0.0

    def _compute_pa_mpjpe_3d(self, pred, gt, valid_mask=None):
        pred_rel = self._make_root_relative(pred)
        gt_rel = self._make_root_relative(gt)
        return self._compute_masked_pa_metric(pred_rel, gt_rel, valid_mask)

    def _compute_root_rigid_mpjpe_3d(self, pred, gt, valid_mask=None):
        pred_rel = self._make_root_relative(pred)
        gt_rel = self._make_root_relative(gt)
        if valid_mask is None:
        ...
        for sample_pred, sample_gt, sample_valid in zip(
            ...
            if finite[0] and finite.sum() >= 3:
                values.append(
                    super()._compute_root_rigid_mpjpe_3d(
                        sample_pred[finite][None], sample_gt[finite][None]
                    )
                )
        if values:
            return torch.stack(values).mean()
        return pred_rel.nan_to_num().sum() * 0.0
    ...
    def _compute_pa_mpvpe(self, pred_vertices, gt_vertices, valid_mask=None):
        """MPVPE after a per-sample similarity Procrustes alignment."""
        return self._compute_masked_pa_metric(
            pred_vertices, gt_vertices, valid_mask=valid_mask
        )
    ...
    @staticmethod
    def _vertex_valid_mask(batch, gt_vertices):
        valid = torch.isfinite(gt_vertices).all(dim=-1)
        if "vertex_valid" not in batch:
            return valid
        batch_valid = batch["vertex_valid"].to(device=gt_vertices.device)
        if batch_valid.ndim == 1:
            batch_valid = batch_valid[:, None]
        if batch_valid.ndim == 3 and batch_valid.shape[-1] == 1:
            batch_valid = batch_valid.squeeze(-1)
        if batch_valid.ndim != 2:
            raise ValueError(
                f"Unsupported vertex_valid mask shape: {tuple(batch_valid.shape)}"
            )
        return valid & batch_valid.bool().expand_as(valid)

    def _mesh_supervision_targets(self, batch):
        if "gt_vertices" not in batch:
            raise KeyError(
                "DexYCB mesh supervision requires batch['gt_vertices'] with "
                "shape [B, 778, 3]"
            )
    ...
    def _compute_losses(self, batch, results, image_size):
        ...
        vertex_valid = self._vertex_valid_mask(batch, gt_vertices_relative)
