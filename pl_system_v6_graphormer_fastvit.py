...
# 替换为：
from models.model_fastvit import FastViTFeatureExtractor
...
class PoseLightningModule(pl.LightningModule):
    def __init__(self, lr=1e-3, num_joints=21, local_model_dir=None, feature_dim=768, layers=[3, 6, -1],
                  upsample_dim=512, num_refine_layers=3, use_gradient_checkpointing=True, backbone_ckpt_path=None): # 默认层数建议设为3
        super().__init__()
    ...
    def forward(self, x, hand_back):
        ...
        # 1. 特征提取 (直接获取列表 [f1, f2, f3])
        extracted_features = self.vitmodel(x)
        
        # 2. 上采样处理
        # 此时 extracted_features 长度为 3，self.upsample_heads 长度也是 3
        upsampled_features = []
        for i, feat in enumerate(extracted_features):
            # 直接按索引匹配，不再需要 layer_id
            up_feat = self.upsample_heads[i](feat)
            upsampled_features.append(up_feat)
    ...
    def _compute_root_aligned_mpjpe_3d(self, pred, gt, root_idx=0):
        """
        计算根节点对齐后的 MPJPE (Root-Aligned MPJPE)
        pred: [B, N, 3]
        gt: [B, N, 3]
        root_idx: 根节点索引，默认 0 代表手腕节点
        """
        # 将预测值和 GT 的根节点平移到原点
        pred_aligned = pred - pred[:, root_idx:root_idx+1, :]
        gt_aligned = gt - gt[:, root_idx:root_idx+1, :]
        
        # 计算欧氏距离并求均值
        dist = torch.norm(pred_aligned - gt_aligned, dim=-1)
        return dist.mean()
    ...
    def test_step(self, batch, batch_idx):
        ...
        # --- 新增: 计算根节点对齐的 MPJPE ---
        test_root_align_mpjpe = self._compute_root_aligned_mpjpe_3d(pred_mu, gt_pose)
        ...
        # --- 新增: 记录到 log ---
        self.log('test_root_align_mpjpe', test_root_align_mpjpe, prog_bar=True)
