...
# from models.model_resnet import ResNetFeatureExtractor
...
class PoseLightningModule(pl.LightningModule):
    # 修改 1：将默认参数 layers=[3, 6, -1] 改为 layers=[-1]，只提取最后一层
    def __init__(self, lr=1e-3, num_joints=21, local_model_dir=None, feature_dim=768, layers=[-1],
                  upsample_dim=512, num_refine_layers=3, use_gradient_checkpointing=True):
        super().__init__()
        ...
        self.vitmodel = ViTFeatureExtractor(
            ...
            layers_to_extract=self.layers,  # 修改 2：原来这里写死了 [3, 6, -1]，改为 self.layers
            freeze_backbone=True
        )
        
        # 2. Upsampling & Fusion
        # 由于 layers 现在只有 [-1]，这里自动只会生成 1 个 UpsampleHead4x
    ...
    def forward(self, x, hand_back):
        ...
        # 修改 4：不使用 fuse_block，直接将最后一层（也是唯一一层）上采样特征作为全局特征图
        # global_feature_map = self.fuse_block(upsampled_features) 
        global_feature_map = upsampled_features[-1]
        ...
        query_pos   = self.joint_token_pos.expand(B, -1, -1) # [B, 21, 256]
