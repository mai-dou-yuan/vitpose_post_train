...
class PoseLightningModule(pl.LightningModule):
    ...
    def forward(self, x, hand_back):
        ...
        # 准备 Back Memory
        back_feature_map = self.back_feature_extractor(hand_back) # torch.Size([12, 512, 42, 42])
        # [B, C, H, W] -> [B, L, C]
        ...
        pos_back = self.pos_embed_back.expand(B, -1, -1)

        # pdb.set_trace()
        # 将 Feature 和 Pos 都拼接
        # combined_memory: [B, L_palm + L_back, C]
        combined_memory = torch.cat([memory_palm, memory_back], dim=1)
        combined_pos    = torch.cat([pos_palm, pos_back], dim=1)
