...
class GatedBackRefinementBlock(nn.Module):
    def __init__(self, d_model, n_head=8, dim_feedforward=None, dropout=0.1):
        ...
        # ====== Part 3: 门控网络 (修改部分) ======
        # 输入维度改为 d_model * 2，因为我们要拼接 curr_tokens 和 t_back_final
        ...
        # 初始偏置保持负数，让模型在训练初期先依赖主干，再慢慢学习利用手背特征
    ...
    def forward(self, curr_tokens, back_memory, query_pos=None, back_pos=None):
        ...
        # ==========================================
        # Step 3: Gated Fusion (门控融合 - 修改部分)
        # ==========================================
        # 将当前 Token 和 提取到的手背特征在通道维度拼接
        # gate_input 维度: [B, N, C * 2]
        gate_input = torch.cat([curr_tokens, t_back_final], dim=-1)
