...
from models.GatedTransformerDecoderLayer import GatedTransformerDecoderLayer
...
class PoseLightningModule(pl.LightningModule):
    def __init__(self, lr=1e-3, num_joints=21, local_model_dir=None, feature_dim=768, layers=[3, 6, -1],
        ...
        # self_attn和corss_attn融合到一起
        self.decoder_layer = nn.ModuleList([
            GatedTransformerDecoderLayer(
        d_model=upsample_dim,
        nhead=8,
        dim_feedforward=1024,
        dropout=0.1,
    ...
    def forward(self, x,):
        ...
        global_feature_map = self.fuse_block(upsampled_features) # torch.Size([16, 512, 96, 96])
        pos_embed_map = self.pos_embed_layer(global_feature_map) # torch.Size([16, 512, 96, 96])
        
        pos_embed_map = pos_embed_map.flatten(2).transpose(1, 2) # shape: [16, 96*96, 512]
        global_feature_map = global_feature_map.flatten(2).transpose(1, 2)  # shape: [16, 96*96, 512]
        # 4. 准备 Transformer 输入
        # Memory Pos Embed
        ...
        for i in range(self.num_refine_layers):
            ...
            # )
            curr_tokens = self.decoder_layer[i](
                tgt=curr_tokens,
                memory=global_feature_map,
                query_pos=None,
                pos_emb=pos_embed_map,
